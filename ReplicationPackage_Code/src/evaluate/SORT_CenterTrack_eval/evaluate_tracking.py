...
from SORT_CenterTrack_eval.munkres import Munkres
...
# from https://github.com/xingyizhou/CenterTrack
class tData:
    """
...
class trackingEvaluation(object):
    ...
    def __init__(self, t_sha, gt_path,
        split_version='', min_overlap=0.5, max_truncation = 0, min_height = 25, 
        max_occlusion = 2, mail=None, cls="car",filename_test_mapping = ''):
    ...
    def compute3rdPartyMetrics(self):
        ...
        for seq_idx in range(len(self.groundtruth)):
            ...
            for f in range(len(seq_gt)):
                ...
                # 打印tmpfp
                if tmpfp!=0:
                    # print(str(f)+" tmpfp: "+str(tmpfp))
                    fp_frame.append(f)
                if tmpfn!=0:
                    # print(str(f)+" tmpfn: "+str(tmpfn))
                    fn_frame.append(f)
            ...
            #  print seq inconsistent
            print("seq: "+str(seq_idx) + " missing extraction: " + str(seqfp)+ " extra extraction: " + str(seqfn))
            out_record.append("seq: "+str(seq_idx) + " missing extraction: " + str(seqfp)+ " extra extraction: " + str(seqfn))
            fp_frame = set(fp_frame)
            fn_frame = set(fn_frame)
            fp_frequency = len(fp_frame)
            fn_frequency = len(fn_frame)
            fp_frames.append(list(fp_frame))
            fn_frames.append(list(fn_frame))

            # print("seq: "+str(seq_idx) +" n_gts: "+str(n_gts))
        ...
        for seq_idx, (seq_trajectories,seq_ignored) in enumerate(zip(self.gt_trajectories, self.ign_trajectories)):
            ...
            n_ignored_tr = 0
            for k, ign_g in zip(seq_trajectories.keys(), seq_ignored.values()):
                # all frame s of this gt trajectory are ignored
                g = seq_trajectories[k]
                if all(ign_g):
                    n_ignored_tr+=1
                ...
                for f in range(1,len(g)):
                    ...
                    if last_id != g[f] and last_id != -1 and g[f] != -1 and g[f-1] != -1:
                        ...
                        # concreat ids
                        # print("key: "+ str(k))
                        # print("seq: "+str(seq_idx)+" f："+str(f)+" g:"+str(g))
                        #out_record.append("seq: "+str(seq_idx)+" f："+str(f)+" g:"+str(g))
                        # idframe = compare_txt.id_frame_query(gt_path, seq_idx, k, f+1)
                        # ids_frame.append(idframe)
                        # print("gt_frame: "+str(idframe))
            ...
            #print sequence ids
            print("seq: "+str(seq_idx) +" ids: "+ str(tmpId_switches))
            out_record.append("seq: "+str(seq_idx) +" ids: "+ str(tmpId_switches))

            ### print fp fn frequency
            print("seq: "+str(seq_idx) + " miss_frequency: "+ str(len(fp_frames[seq_idx])) + " miss_frame: "+str(fp_frames[seq_idx]))
            print("seq: "+str(seq_idx) + " extra_frequency: "+ str(len(fn_frames[seq_idx])) + " extra_frame: "+str(fn_frames[seq_idx]))
            ids_frame = set(ids_frame)
            ids_frequency = len(ids_frame)
            # print("seq: " + str(seq_idx) + " ids_frequency:" + str(ids_frequency) + " id_frame:"+ str(ids_frame))
            error_frame = ids_frame.union(set(fp_frames[seq_idx]),set(fn_frames[seq_idx]))
            error_frequency = len(error_frame)
            print("seq: "+str(seq_idx) + " violation_frequency: "+ str(error_frequency))
    ...
    def createSummary(self):
        ...
        summary += self.printEntry("Missing extration", self.fp) + "\n"
        summary += self.printEntry("Extra extraction:", self.fn) + "\n"
...
def evaluate(result_sha,source_path, mail, split_version='',filename_test_mapping=''):
    """
    ...
    for c in ("car", "pedestrian"):
        ...
        e = trackingEvaluation(
            t_sha=result_sha, gt_path = source_path, mail=mail,cls=c,split_version=split_version,filename_test_mapping=filename_test_mapping)
...
if __name__ == "__main__":
    ...
    #gt_path = 'D:/exp/SORT/txt_results/mosaic_random_3_source/'
    gt_path = 'D:/exp/CenterTrack/txt_results/mosaic_random_3_source_new/'
    result_sha = "D:/exp/CenterTrack/txt_results/mosaic_random_3_follow_new/"
    #result_sha = 'D:/exp/SORT/txt_results/mosaic_random_3_follow/'
    split_version = "test"
    test_seqmap = "D:/Code/Python/track/eval_kitti_track/CenterTrack_test.seqmap"
    mail = mailpy.Mail("")
    success = evaluate(result_sha,gt_path,mail,split_version=split_version,filename_test_mapping = test_seqmap)
