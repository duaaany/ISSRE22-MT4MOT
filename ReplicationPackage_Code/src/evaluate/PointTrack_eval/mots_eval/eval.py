...
# from https://github.com/detectRecog/PointTrack
...
def compute_MOTS_metrics_per_sequence(seq_name, gt_seq, results_seq, max_frames, class_id,
                                      ignore_class, overlap_function, gt_folder):
    ...
    for f in range(max_frames + 1):
        ...
        # print(str(g)+str(num_associations))
        ...
        # print(str(t)+str(tmptp)+str(nignoredtracker))
    ...
    if len(seq_trajectories) != 0:
        ids_frame = []
        ids_frequency = 0
        for k in seq_trajectories.keys():
            ...
            for f in range(1, len(g)):
                if last_id != g[f] and last_id != -1 and g[f] != -1:
                    ...
                    # print id switch
                    # print("seq: "+str(seq_name)+" id switch: f:"+str(f)+" g:"+str(g))
                    idframe = compare_txt.id_frame_query(gt_folder, int(seq_name), k, f + 1)
        ...
        print("seq: "+ str(seq_name)+" ids_frenquency: " + str(ids_frequency)+ " ids_frame: "+str(ids_frame))
        # print("seq: " + str(seq_name) + " ids_frenquency: " + str(ids_frequency))

    return results_obj


def compute_MOTS_metrics(gt, results, max_frames, class_id, ignore_class, overlap_function,gt_floder):
    """
    ...
    for seq in gt.keys():
        ...
        results_per_seq[seq] = compute_MOTS_metrics_per_sequence(seq, gt[seq], results_seq, max_frames[seq], class_id,
                                                                 ignore_class, overlap_function,gt_floder)
...
def print_summary(seq_names, results_per_seq, results_for_all_seqs, column_width=5):
    ...
    metrics = [("Frames", "total_num_frames"), ("TP", "tp"), ("Missing","fp"), ("Extra", "fn"), ("IDS", "id_switches")]
...
def evaluate_class(gt, results, max_frames, class_id,gt_folder):
  _, results_obj = compute_MOTS_metrics(gt, results, max_frames, class_id, IGNORE_CLASS, mask_iou,gt_folder)
  return results_obj
...
def run_eval(results_folder, gt_folder, seqmap_filename):
  ...
  results_cars = evaluate_class(gt, results, max_frames, 1,gt_folder)
...
if __name__ == "__main__":
  ...
  #   sys.exit(1)
  results_folder = "D:/exp/PointTrack/txt_result/random/noise_random_3_follow/"
  gt_folder = "D:/exp/PointTrack/txt_result/random/noise_random_3_source/"
  seqmap_filename = "D:/Code/Python/track/eval_kitti_track/CenterTrack_test.seqmap"
